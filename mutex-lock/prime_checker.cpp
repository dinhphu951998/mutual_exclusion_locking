...
static constexpr int NUM_OF_ITERS = 10000; // keep it the same for every locking, easier to analyze the result
static const int THREAD_COUNTS[] = {1, 2, 4, 8, 12, 16, 20}; //current laptop total thread is 20
static const int OUTSIDE_WORKS[] = {0, 1, 3, 5};
static const int NS[] = {51, 89};//{  101, 503, 1009, 10007, 50021, 100003  };
...
static double time_tas_ms(int num_threads, int outside_work, int n)
{
    tas_spinlock mtx;
...
static double time_ttas_ms(int num_threads, int outside_work, int n)
{
    ttas_spinlock mtx;
...
int main()
{
    constexpr size_t n_tc = sizeof(THREAD_COUNTS) / sizeof(THREAD_COUNTS[0]);
    constexpr size_t n_ow = sizeof(OUTSIDE_WORKS) / sizeof(OUTSIDE_WORKS[0]);
    constexpr size_t n_n = sizeof(NS) / sizeof(NS[0]);

    cout << "threads,num_iters,outside_work,n,tas_ms,ttas_ms,mcs_ms\n";
    ...
    for (size_t a = 0; a < n_tc; ++a) {
        ...
        for (size_t b = 0; b < n_ow; ++b) {
            ...
            for (size_t c = 0; c < n_n; ++c) {
                ...
                double tas_ms = time_tas_ms(num_threads, outside_work, n);
                double ttas_ms = time_ttas_ms(num_threads, outside_work, n);
                double mcs_ms = time_mcs_ms(num_threads, outside_work, n);
